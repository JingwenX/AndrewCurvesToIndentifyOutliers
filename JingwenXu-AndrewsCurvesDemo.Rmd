...
Abstract: Abnormal data (outliers) would be a headache when using statistical analysis. For a project that uses gene expression profile to study the effect of molecules, the dimension could be very high, at which point it would be hard to identify single outliers. 

If the baseline gene expression data has some abnormal, if would affect the later study of the data. Thus here I use one high dimensional data visualization tool "Andrews Curves" to identify the abnormal experimental unit at baseline, in order to clean the data for further use.
